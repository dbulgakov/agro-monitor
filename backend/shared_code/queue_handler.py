...
def process_analysis(msg: func.QueueMessage):
    ...
    geom_buf = _buffer_geom(payload.area.geometry.model_dump(), meters=30)
    ...
    try:
        ...
        logging.warning(f"Recommendations failed for job {jid}: {e}", exc_info=True)
    ...
    try:
        ...
        report_blob.upload_blob(
            rpt.model_dump_json(indent=2, exclude_none=True).encode('utf-8'),
            overwrite=True,
