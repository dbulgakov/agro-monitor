...
def _upload(client, jid, buf, name):
    try:
        ...
        blob.upload_blob(buf, overwrite=True, content_settings=ContentSettings(content_type='image/png'))
        return blob.url
    ...
    try:
        with Env(GDAL_HTTP_TIMEOUT=60):
            with rasterio.open(href) as src:
                ...
                out, tfm = mask(src, [geom], crop=True, indexes=1, filled=True)
                data = out[0]
                if data.size == 0 or (src.nodata is not None and np.all(data == src.nodata)):
                    raise RuntimeError('No data')
                return data, tfm, src.crs
    except Exception as e:
        logging.warning(f"Band read failed {href}: {e}")
        return _blank_gray(), None, None

def _down(arr):
    if DOWNSAMPLE <= 1 or arr.size <= 1:
        return arr
    h, w = arr.shape[:2]
    ht, wt = h - h % DOWNSAMPLE, w - w % DOWNSAMPLE
    if ht == 0 or wt == 0:
        return arr
    return arr[:ht, :wt].reshape(ht // DOWNSAMPLE, DOWNSAMPLE, wt // DOWNSAMPLE, DOWNSAMPLE).mean((1, 3))
...
def _fetch_scenes(payload):
    ...
    for item in search.item_collection():
        itm = sign(item)
        urls = {}
        for k, opts in {'nir': ['B08_20m', 'B08'], 'red': ['B04_20m', 'B04']}.items():
            for o in opts:
                if o in itm.assets:
                    urls[k] = itm.assets[o].href
                    break
        urls['visual'] = itm.assets.get('visual', itm.assets.get('B04_20m', {})).href
        scenes.append(urls)
    return scenes
...
def process_analysis(msg: func.QueueMessage):
    ...
    geo = payload.area.geometry.model_dump()
    use = None

    for idx, urls in enumerate(scenes, 1):
        metadata['sceneIndex'] = idx
        update_job_status(client, jid, JobStatus.PROCESSING, 20, 'Читання інфрачервоного спектру', raw=json.dumps(metadata))
        nir, _, _ = _read_band(urls.get('nir', ''), geo)
        metadata['nirPixels'] = int(nir.size)

        update_job_status(client, jid, JobStatus.PROCESSING, 30, 'Читання червоного спектру', raw=json.dumps(metadata))
        red, _, _ = _read_band(urls.get('red', ''), geo)
        metadata['redPixels'] = int(red.size)

        nir, red = _down(nir), _down(red)
        if nir.size > 1 and red.size > 1:
            use = urls
            break

    if use is None and scenes:
        use = scenes[0]
        metadata['usingFallbackScene'] = True
    elif use:
        metadata['usingFallbackScene'] = False
    else:
        logging.error(f"No scenes found for job {jid}")
    ...
    nir, _, _ = _read_band(use.get('nir', ''), geo)
    red, _, _ = _read_band(use.get('red', ''), geo)
    nir, red = _down(nir), _down(red)
    ndvi = _ndvi(nir, red)
    ...
    if 'visual' in use:
        ...
        v, _, _ = _read_band(use['visual'], geo)
        v = _down(v)
        metadata['rgbPixels'] = int(v.size)
        if v.size > 1:
            vis_url = _upload(client, jid, _to_png(v, False), 'rgb.png')
