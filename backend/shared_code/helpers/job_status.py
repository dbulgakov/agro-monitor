...
async def get_job_status(client: BlobServiceClient, job_id: str) -> Optional[ProgressUpdate]:
    log_adapter = logging.getLogger(__name__).getChild(job_id)
    try:
        blob = client.get_blob_client(container=REPORTS_CONTAINER_NAME, blob=f"{job_id}/status.json")
...
async def update_job_status(client: BlobServiceClient, job_id: str, status: JobStatus, progress: int, message: str = None):
    log_adapter = logging.getLogger(__name__).getChild(job_id)
    try:
        blob = client.get_blob_client(container=REPORTS_CONTAINER_NAME, blob=f"{job_id}/status.json")
