...
from functions.lib.helpers.blob import upload_image_to_blob, upload_report_to_blob
...
async def process_analysis(msg: func.QueueMessage):
    ...
    try:
        ...
        await update_status(job_id, JobStatus.FAILED, -1, f"Invalid payload: {err}")
        return
    ...
    try:
        await update_status(job_id, JobStatus.PROCESSING, 10, "Acquiring data")
        urls = await fetch_band_urls(job_id, payload)

        await update_status(job_id, JobStatus.PROCESSING, 30, "Reading and computing NDVI")
        ndvi, rgb = await read_and_compute_ndvi(job_id, urls)

        await update_status(job_id, JobStatus.PROCESSING, 60, "Creating visualizations")
        ndvi_buf = create_ndvi_buffer(ndvi)
        ndvi_url = await upload_image_to_blob(job_id, ndvi_buf, "ndvi_map.png")
        rgb_url = None
        if rgb is not None:
            rgb_buf = create_rgb_buffer(rgb)
            rgb_url = await upload_image_to_blob(job_id, rgb_buf, "rgb_map.png")

        await update_status(job_id, JobStatus.PROCESSING, 80, "Generating recommendations")
        ...
        await update_status(job_id, JobStatus.PROCESSING, 95, "Finalizing report")
        stats = {
        ...
        await update_status(job_id, JobStatus.COMPLETED, 100, "Analysis complete")
        logger.info("Job completed successfully.")

    except Exception as err:
        logger.error(f"Processing error: {err}", exc_info=True)
        await update_status(job_id, JobStatus.FAILED, -1, f"Error: {err}")
